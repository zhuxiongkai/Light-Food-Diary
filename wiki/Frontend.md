...
- 工具函数放在 `src/utils/`，如 `aiService.ts`（AI 识别）、`servingSize.ts`（份量配置）、`exportService.ts`（数据导出）、`renderMarkdown.ts`（Markdown 渲染）、`mealTime.ts`（时间→餐食类型）、`inlineSvgIcons.ts`（内联 SVG 图标）
